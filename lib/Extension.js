!function(){
	...
	module.exports = new Class({
        ...
        , onBeforePrepare: function(resource, definition) {
            ...
            if (resource._hasDeletedColumn && resource.getQueryMode() === resource.MODE_SELECT && resource.isRootResource() && !resource._dontFilterSoftDeleted) {
        ...
        , onBeforePrepareSubqueries: function(resource, definition) {
            ...
            // check if we need to apply the not deleted filter
            if (resource._hasDeletedColumn && resource.getQueryMode() === resource.MODE_SELECT && !resource.isRootResource() && !resource.getRootResoure()._dontFilterSoftDeleted) {
                // aply soft dleete filter
                if (!resource.query.filter[name]) resource.query.filter[name] = {};
                resource.query.filter[name][this.deleted] = null;
            }
